...
public class ToolkitUtil {
  ...
  public static void runWhenProjectIsReady(
  ...
  public static void invokeLater(
    ...
    if (isNoBackgroundMode()) {
      ...
      ApplicationManager
        .getApplication()
        .invokeLater(DisposeAwareRunnable.create(r, p), state);
    }
  }
  ...
  public static PsiClass findPsiClass(
  ...
  public static String textToRequestParam(String text) {
    ...
    if (paramMap.size() > 0) {
      paramMap.forEach((s, o) -> {
        try {
          param.append(s).append("=").append(URLEncoder.encode(o, "UTF-8")).append("&");
